...
def test_files_path(tmp_path):
    src_path = "/home/xarth/codes/xarthisius/pyempadcalibratescript/related_data/EMPAD2-calib_oct2020"
    data_path = tmp_path / "data"
    data_path.mkdir()
    background_path = data_path / "background.bin"
    shutil.copy(os.path.join(src_path, "bkgd_reduced.raw"), background_path)

    raw_path = data_path / "raw.bin"
    shutil.copy(os.path.join(src_path, "raw_reduced.raw"), raw_path)
...
def test_main(test_files_path):
    ...
    with runner.isolated_filesystem(temp_dir=test_files_path) as td:
        ...
        assert "Background shape: (128, 128, 10000)" in result.output
        assert "Raw data shape: (128, 128, 10000)" in result.output
